...
private:
    ...
    public:
        ...
        // eigenen Schaltzustaenden - es kennt den Editor nicht.
        int activeThemeIdx() const
        {
            for (int i = 0; i < kThemes; ++i)
                if (themeBtn[i].getToggleState()) return i;
            return 0;
        }
        int activeLayoutIdx() const { return layoutBtn[1].getToggleState() ? 1 : 0; }
